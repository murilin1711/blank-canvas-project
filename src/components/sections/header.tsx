      ...
      <header className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'top-0' : ''}`}>
                  ...
                  {navItems.map((item) => (
                    <li key={item} className="relative group">
                      {item === "Sobre" ? (
                        <Link
                          to="/sobre"
                          className="font-suisse text-body-regular text-foreground hover:bg-background/80 h-[34px] px-3 rounded-lg transition-all duration-300 whitespace-nowrap group-hover:scale-105 group-hover:shadow-sm flex items-center"
                        >
                          {item}
                        </Link>
                      ) : (
                        <button
                          ...
                          className="font-suisse text-body-regular text-foreground hover:bg-background/80 h-[34px] px-3 rounded-lg transition-all duration-300 whitespace-nowrap group-hover:scale-105 group-hover:shadow-sm"
                        >
                          {item}
                        </button>
                      )}
                      <div className="absolute inset-x-0 -bottom-1 h-0.5 bg-primary opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-x-0 group-hover:scale-x-100" />
                    </li>
                  ))}
                </ul>
              </nav>
                  ...
                  {searchOpen ? (
                    <div className="flex items-center h-full px-3 gap-2">
                      <input
                        ...
                        placeholder="Buscar..."
                        autoFocus
                        className="flex-1 bg-transparent border-none outline-none text-body-regular text-foreground placeholder:text-muted-foreground"
                        onBlur={() => setSearchOpen(false)}
                      />
                      <Search className="w-[14px] h-[14px] text-foreground flex-shrink-0" />
                    </div>
                  ) : (
                    <button onClick={() => setSearchOpen(true)} className="w-full h-full flex items-center justify-between px-3">
                      <span className="font-suisse text-body-regular text-foreground">Buscar</span>
                      <Search className="w-[14px] h-[14px] text-foreground transition-transform duration-300 group-hover:scale-110" />
                    </button>
                  )}
                </div>
              </div>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden fixed inset-0 z-40 pt-[80px]">
            ...
            <div className="relative bg-background/95 backdrop-blur-xl w-full max-w-sm mx-auto rounded-b-3xl shadow-2xl p-6">
              <nav>
                    ...
                    <li key={item}>
                      {item === "Sobre" ? (
                        <Link
                          ...
                          className="block font-suisse text-body-lg font-medium text-foreground py-3 px-4 rounded-lg hover:bg-muted transition-colors duration-300 hover:scale-105 hover:shadow-sm w-full text-left"
                        >
                          {item}
                        </Link>
                      ) : item === "Escolas" ? (
                        <>
                          <button
                            ...
                            className="block w-full text-left font-suisse text-body-lg font-medium text-foreground py-3 px-4 rounded-lg hover:bg-muted transition-colors duration-300 flex justify-between items-center"
                          >
                            {item}
                            ...
                            <ul className="pl-6 mt-2 space-y-2">
                              <li>
                                <Link
                                  ...
                                  onClick={() => setMobileMenuOpen(false)}
                                  className="block text-primary font-bold text-body-regular hover:underline"
                                >
                                  Colégio Militar
                                </Link>
                              </li>
        ...
        <div
          ...
          className="hidden lg:block absolute top-20 left-8 xl:left-12 2xl:left-16 right-8 xl:right-12 2xl:right-16 rounded-xl bg-background/85 backdrop-blur-[20px] opacity-0 max-h-0 data-[menu-open=true]:opacity-100 data-[menu-open=true]:max-h-96 overflow-hidden transition-[max-height,opacity] duration-300 ease-[cubic-bezier(.16,1,.3,1)] shadow-xl z-[55] border border-border/20"
        >
              ...
              >
                {item === "Escolas" ? (
                  <div className="flex flex-col items-center gap-4">
                    <Link
                      to="/escolas/colegio-militar"
                      className="text-primary font-bold text-body-lg hover:underline"
                    >
                      Colégio Militar
                    </Link>
                  </div>
                  ...
                  <div className="text-center">
                    <h3 className="text-h3 text-primary mb-4">{item}</h3>
                    <p className="text-muted-foreground">Conteúdo do submenu para {item}</p>
                  </div>
                )}
              </div>
            ))}
