...
async function getValidToken(supabase: any): Promise<string> {
  ...
  const now = Date.now();
  if (_tokenCache && _tokenCache.expiresAt > now + bufferMs) return _tokenCache.token;
...
function validateAdminToken(token: string): boolean {
  try {
    ...
    return (parts[0] === "admin" || parts[0] === "caixa") && Date.now() < parseInt(parts[1]);
  } catch {
    return false;
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "quote") {
      ...
      const res = await meFetch("/me/shipment/calculate", meToken, {
        method: "POST",
    ...
    if (action === "generate") {
      ...
      const cartBody = {
        ...
        from: {
          ...
          company_document: (meProfile.company_document || "01184449000110").replace(/\D/g, ""),
      ...
      const cartRes = await meFetch("/me/cart", meToken, {
        method: "POST",
        body: JSON.stringify(cartBody),
      });

      if (!cartRes.ok) throw new Error(`Erro ao adicionar ao carrinho ME: ${await cartRes.text()}`);
      ...
      if (!checkoutRes.ok) throw new Error(`Erro no checkout ME (verifique saldo): ${await checkoutRes.text()}`);
      ...
      if (!generateRes.ok) throw new Error(`Erro ao gerar etiqueta ME: ${await generateRes.text()}`);

      const generateData = await generateRes.json();
      const trackingCode = generateData?.[meOrderId]?.tracking || generateData?.tracking || null;

      const printRes = await meFetch(`/me/shipment/print?orders[]=${meOrderId}&mode=public`, meToken, {
        method: "GET",
      });
      ...
      if (printRes.ok) {
        if (printRes.redirected) {
          ...
          if (ct.includes("application/json")) {
            const pd = await printRes.json();
            labelUrl = pd?.url || pd?.link || null;
          } else {
    ...
    return new Response(
      JSON.stringify({ error: "Ação inválida. Use: quote ou generate" }),
